...
TEST_PROFILE_DIRECTORY=$(get_test_profile_directory "input" "regfinfo");
...
";
	...
	then
		...
		do
			run_test_on_input_file_with_options "${TEST_SET_DIRECTORY}" "regfinfo" "with_stdout_reference" "${OPTION_SETS}" "${TEST_EXECUTABLE}" "${INPUT_FILE}" "${OPTIONS[@]}";
			RESULT=$?;
