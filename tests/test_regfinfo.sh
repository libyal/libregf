...
TEST_EXECUTABLE="../regftools/regfinfo";

if ! test -x "${TEST_EXECUTABLE}";
then
	TEST_EXECUTABLE="../regftools/regfinfo.exe";
fi
		...
		do
			...
			do
				...
				then
					...
					run_test_on_input_file "${TEST_SET_DIRECTORY}" "regfinfo" "with_stdout_reference" "${OPTION_SET}" "${TEST_EXECUTABLE}" "${INPUT_FILE}" "${PROFILE_OPTIONS[@]}" "${OPTIONS[@]}";
					RESULT=$?;
			...
			then
				run_test_on_input_file "${TEST_SET_DIRECTORY}" "regfinfo" "with_stdout_reference" "" "${TEST_EXECUTABLE}" "${INPUT_FILE}" "${PROFILE_OPTIONS[@]}";
				RESULT=$?;
			fi
