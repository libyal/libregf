...
def pyregf_test_single_open_close_file_object_with_dereference( filename, mode ):
	file_object = open( filename, mode )
	regf_file = pyregf.file()
	regf_file.open_file_object( file_object, mode )
	del file_object
...
def main( argc, argv ):
	...
	if argc < 2:
		print "Usage: pyregf_test_open_close.py filename\n"
		return 1

	print "Testing single open close of: %s with access: read\t" %( sys.argv[ 1 ] ),

	try:
		pyregf_test_single_open_close_file( argv[ 1 ], "r" )
		result = 0
	except:
		result = 1
	...
	result = 1
	try:
		pyregf_test_single_open_close_file( None, "r" )
	except TypeError, exception:
		if exception.message == "pyregf_file_open: unsupported string object type":
			result = 0
	except:
		pass
	...
	result = 1
	try:
		pyregf_test_single_open_close_file( argv[ 1 ], "w" )
	except ValueError, exception:
		if exception.message == "pyregf_file_open: unsupported mode: w.":
			result = 0
	except:
		pass
	...
	try:
		pyregf_test_multi_open_close_file( argv[ 1 ], "r" )
		result = 0
	except:
		result = 1
	...
	try:
		pyregf_test_single_open_close_file_object( argv[ 1 ], "r" )
		result = 0
	except:
		result = 1
	...
	try:
		pyregf_test_single_open_close_file_object_with_dereference( argv[ 1 ], "r" )
		result = 0
	except:
		result = 1
	...
	try:
		pyregf_test_multi_open_close_file_object( argv[ 1 ], "r" )
		result = 0
	except:
		result = 1
