...
then
	docker run -t -v "${PWD}:/libregf" ${DOCKERHUB_REPO}:${DOCKERHUB_TAG} sh -c "cd libregf && .travis/script_docker.sh";
...
then
	...
	then
		...
		cp AUTHORS COPYING COPYING.LESSER NEWS README ${PWD}/osx-pkg/usr/share/doc/libregf;
