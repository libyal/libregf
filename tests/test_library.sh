...
LIBRARY_TESTS="checksum data_block_key data_block_stream data_type error file_header hive_bin hive_bin_cell hive_bin_header hive_bins_list io_handle key key_descriptor key_item multi_string named_key notify security_key sub_key_list value value_item value_key";
...
run_test()
{
	...
	local TEST_EXECUTABLE="./regf_test_${TEST_NAME}";
...
run_test_with_input()
{
	...
	local TEST_PROFILE_DIRECTORY=$(get_test_profile_directory "input" "libregf");
