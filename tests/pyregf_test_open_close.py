...
def pyregf_test_single_open_close_file(filename, mode):
  ...
  try:
    ...
    if (not filename and
        exception.message == "pyregf_file_open: unsupported string object type."):
      pass
    ...
    if (mode == "w" and
        exception.message == "pyregf_file_open: unsupported mode: w."):
      pass
...
def main():
  ...
  if not pyregf_test_single_open_close_file(options.source, "r"):
    return False

  if not pyregf_test_single_open_close_file(None, "r"):
    return False

  if not pyregf_test_single_open_close_file(options.source, "w"):
    return False

  if not pyregf_test_multi_open_close_file(options.source, "r"):
    return False

  if not pyregf_test_single_open_close_file_object(options.source, "r"):
    return False

  if not pyregf_test_single_open_close_file_object_with_dereference(
      options.source, "r"):
    return False

  if not pyregf_test_multi_open_close_file_object(
      options.source, "r"):
    return False
