...
Foreach (${TestName} in ${ToolsTests} -split " ")
{
	...
	$Result = RunTestBinary ${TestExecutablesDirectory} "regf_test_tools_${TestName}"
...
For ($ProfileIndex = 0; $ProfileIndex -le ($Profiles.length - 1); $ProfileIndex += 1)
{
	...
	{
		$Result = RunToolsBinaryAndCompareStdout ${TestExecutablesDirectory} "regfinfo" ${TestProfile} "" ${TestInput}
