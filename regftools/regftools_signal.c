...
void (*regftools_signal_signal_handler)( regftools_signal_t ) = NULL;
             ...
             unsigned long signal )
{
	...
	switch( signal )
	{
		...
		case CTRL_C_EVENT:
			if( regftools_signal_signal_handler != NULL )
			{
				regftools_signal_signal_handler(
				 signal );
			}
			return( TRUE );
...
{
	static char *function = "regftools_signal_attach";
...
#if defined( _MSC_VER )
	regftools_signal_initialize_memory_debug();
#endif
     ...
     libcerror_error_t **error )
{
	...
	regftools_signal_signal_handler = NULL;
