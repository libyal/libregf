...
#include "../regftools/info_handle.h"

/* Tests the info_handle_initialize function
 * Returns 1 if successful or 0 if not
 */
int regf_test_tools_info_handle_initialize(
     void )
{
 ...
 * Returns 1 if successful or 0 if not
 */
int regf_test_tools_info_handle_free(
     void )
{
	libcerror_error_t *error = NULL;
	int result               = 0;
...
#endif
{
	...
	REGF_TEST_RUN(
	 ...
	 regf_test_tools_info_handle_initialize );

	REGF_TEST_RUN(
	 "info_handle_free",
	 regf_test_tools_info_handle_free );
